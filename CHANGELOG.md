...
- `servy revert <module>`: reads `/var/lib/servy/manifest.json` and builds a typed inverse plan for a single module (apt list files, apt keyrings, sysctl drop-ins, sshd directive lines, swapfile + `/etc/fstab` line, and — with `--purge-packages` — apt package removal + `systemctl disable --now`). Deploy user removal and group memberships are out of scope in v1 and surface as visible `will_skip` steps. Dry-run and `--yes` gating mirror `servy apply`.
- `internal/manifest`: JSON-on-disk manifest with `schemaVersion: "1"` at `/var/lib/servy/manifest.json` (mode 0600, atomic staged rewrite). Every successful `apply` appends an entry recording apt packages installed, apt list files, apt keyrings, systemd services enabled, sysctl drop-ins, sshd directive lines, swapfile path, `/etc/fstab` line, users created, and groups joined.
- `internal/revert`: pure planner that turns a `manifest.ModuleRecord` into a `plan.Plan`. Package removal / service disablement are opt-in via `Options.PurgePackages`; file removals are unconditional. Ordering: stop services → apt purge → remove apt lists + keyrings → `apt-get update` → sysctl drop-ins + reload → sshd drop-in lines → swapoff + swapfile removal → fstab line.
- `safeops.RemoveSSHDDropInLines` and `safeops.RemoveFstabLine`: FD-anchored, symlink-safe deletions. Both open the parent directory with `O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC`, operate via `Openat`, and swap the rewritten file in with `Renameat`. Empty drop-ins are Unlinkat'd; `sshd -t` failures revert through the same directory FD. Missing files are treated as already reverted (idempotent).
- Hidden `servy internal remove-sshd-dropin-lines --line ...` and `servy internal remove-fstab-line --line ...` subcommands so revert plans can invoke the safeops helpers with argv-based exec.
- `plan.Plan.HasRunnableSteps()` used by revert to detect empty plans before demanding `--yes`.
