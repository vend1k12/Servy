...
| SSH access | Hardening lockout (`PermitRootLogin`, `PasswordAuthentication`, `AllowUsers`) | Each option needs its own `confirmations.disable*` / `restrictSSHUsers`. `WriteSSHDDropIn` runs `sshd -t` before reload; failure reverts the drop-in file through the same directory FD. | The maintainer console access on the VPS if `sshd -t` passes but the deploy user cannot actually log in. |
| `authorized_keys` and sshd drop-in files | Symlink attack, TOCTOU | `AppendAuthorizedKey` and `WriteSSHDDropIn` open the parent directory with `O_DIRECTORY\|O_NOFOLLOW\|O_CLOEXEC`, act via `Openat` with `O_NOFOLLOW`, refuse non-regular targets, and (for the drop-in) stage a sibling tempfile and swap it in with `Renameat`. `sshd -t` rollback uses the same directory FD. | An attacker who already has write access to `~deploy` or `/etc/ssh/sshd_config.d` before Servy runs. |
